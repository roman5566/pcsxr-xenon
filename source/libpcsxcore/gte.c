...
static inline u32 MFC2(int reg) {
	switch (reg) {
		...
		case 10:
		case 11:
			psxRegs.CP2D.p[reg].sd = (s32) psxRegs.CP2D.p[reg].sw.l;
			break;
		...
		case 15:
			psxRegs.CP2D.p[reg].d = gteSXY2;
			break;

		case 28:
		case 29:
			psxRegs.CP2D.p[reg].d = LIM(gteIR1 >> 7, 0x1f, 0, 0) |
...
void gteMFC2() {
	if (!_Rt_) return;
	psxRegs.GPR.p[_Rt_].d = MFC2(_Rd_);
}

void gteCFC2() {
	if (!_Rt_) return;
	psxRegs.GPR.p[_Rt_].d = psxRegs.CP2C.p[_Rd_].d;
}

void gteMTC2() {
	MTC2(psxRegs.GPR.p[_Rt_].d, _Rd_);
}

void gteCTC2() {
	CTC2(psxRegs.GPR.p[_Rt_].d, _Rd_);
}

#define _oB_ (psxRegs.GPR.p[_Rs_].d + _Imm_)
