...
#define VX1  (psxRegs.CP2D.p[2].sw.l)
#define VY1  (psxRegs.CP2D.p[2].sw.h)
#define VZ1  (psxRegs.CP2D.p[3].sw.l)
#define VXY2 (psxRegs.CP2D.r[4])
#define VX2  (psxRegs.CP2D.p[4].sw.l)
#define VY2  (psxRegs.CP2D.p[4].sw.h)
#define VZ2  (psxRegs.CP2D.p[5].sw.l)
...
#define MAC0  (((s32 *)psxRegs.CP2D.r)[24])
#define MAC1  (((s32 *)psxRegs.CP2D.r)[25])
#define MAC2  (((s32 *)psxRegs.CP2D.r)[26])
#define MAC3  (((s32 *)psxRegs.CP2D.r)[27])
#define IRGB  (psxRegs.CP2D.r[28])
#define ORGB  (psxRegs.CP2D.r[29])
#define LZCS  (psxRegs.CP2D.r[30])
#define LZCR  (psxRegs.CP2D.r[31])

#define R11R12 (((s32 *)psxRegs.CP2C.r)[0])
#define R22R23 (((s32 *)psxRegs.CP2C.r)[2])
...
#define TRX (((s32 *)psxRegs.CP2C.r)[5])
#define TRY (((s32 *)psxRegs.CP2C.r)[6])
#define TRZ (((s32 *)psxRegs.CP2C.r)[7])
...
#define RBK (((s32 *)psxRegs.CP2C.r)[13])
#define GBK (((s32 *)psxRegs.CP2C.r)[14])
#define BBK (((s32 *)psxRegs.CP2C.r)[15])
...
#define RFC (((s32 *)psxRegs.CP2C.r)[21])
#define GFC (((s32 *)psxRegs.CP2C.r)[22])
#define BFC (((s32 *)psxRegs.CP2C.r)[23])
#define OFX (((s32 *)psxRegs.CP2C.r)[24])
#define OFY (((s32 *)psxRegs.CP2C.r)[25])
#define H   (psxRegs.CP2C.p[26].sw.l)
#define DQA (psxRegs.CP2C.p[27].sw.l)
#define DQB (((s32 *)psxRegs.CP2C.r)[28])
...
UINT32 getcp2dr(int reg) {
    switch (reg) {
        ...
        case 15:
            psxRegs.CP2D.r[reg] = SXY2;
            break;

        case 28:
        case 29:
            psxRegs.CP2D.r[reg] = LIM(IR1 >> 7, 0x1f, 0, 0) | (LIM(IR2 >> 7, 0x1f, 0, 0) << 5) | (LIM(IR3 >> 7, 0x1f, 0, 0) << 10);
            break;
    }

    GTELOG( "get CP2DR%u=%08x", reg, psxRegs.CP2D.r[reg]);
...
UINT32 getcp2cr(int reg) {
    GTELOG("get CP2CR%u=%08x", reg, psxRegs.CP2D.r[reg]);
...
UINT32 gte_divide(INT16 numerator, UINT16 denominator) {
...
#define _MFC2(x) getcp2dr(x)
#define _MTC2(x,y) setcp2dr(x,y)
#define _CTC2(x,y) setcp2cr(x,y)

// wrapper between mame and pcsx code

void gteMFC2() {
    if (!_Rt_) return;
    psxRegs.GPR.r[_Rt_] = _MFC2(_Rd_);
}
...
void gteMTC2() {
    _MTC2(psxRegs.GPR.r[_Rt_], _Rd_);
}

void gteCTC2() {
    _CTC2(psxRegs.GPR.r[_Rt_], _Rd_);
}

#define _oB_ (psxRegs.GPR.r[_Rs_] + _Imm_)

void gteLWC2() {
    _MTC2(psxMemRead32(_oB_), _Rt_);
}

void gteSWC2() {
    psxMemWrite32(_oB_, _MFC2(_Rt_));
}
...
void gteRTPS() {
    ...
    SZ2 = SZ3;
    SZ3 = Lm_D( MAC3 );
    int h_over_sz3 = Lm_E( gte_divide( H, SZ3 ) );
    SXY0 = SXY1;
    SXY1 = SXY2;
    SX2 = Lm_G1( F( (INT64) OFX + ( (INT64) IR1 * h_over_sz3 ) ) >> 16 );
    SY2 = Lm_G2( F( (INT64) OFY + ( (INT64) IR2 * h_over_sz3 ) ) >> 16 );
...
void gteDCPL(){
    ...
    s64 RIR1 = ((s64)R * IR1) >> 8;
    s64 GIR2 = ((s64)G * IR2) >> 8;
    s64 BIR3 = ((s64)B * IR3) >> 8;
    ...
    MAC1 = A1(RIR1 + ((IR0 * Lm_B1(RFC - RIR1, 0)) >> 12));
    MAC2 = A2(GIR2 + ((IR0 * Lm_B1(GFC - GIR2, 0)) >> 12));
    MAC3 = A3(BIR3 + ((IR0 * Lm_B1(BFC - BIR3, 0)) >> 12));
