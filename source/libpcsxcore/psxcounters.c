...
void psxRcntUpdate() {
	...
	if (cycle - rcnts[3].cycleStart >= rcnts[3].cycle) {
		...
		if (hSyncCount == VBlankStart) {
			HW_GPU_STATUS &= SWAPu32(~PSXGPU_LCF);
			GPU_vBlank(1, 0);
			//            GPU_vBlank(1);
			setIrq(0x01);
		...
		if (hSyncCount >= (Config.VSyncWA ? HSyncTotal[Config.PsxType] / BIAS : HSyncTotal[Config.PsxType])) {
			...
			gpuSyncPluginSR();
			if ((SWAPu32(HW_GPU_STATUS) & PSXGPU_ILACE_BITS) == PSXGPU_ILACE_BITS)
				HW_GPU_STATUS |= SWAPu32(frame_counter << 31);
			GPU_vBlank(0, SWAPu32(HW_GPU_STATUS >> 31));
			//GPU_vBlank(0);
		}
