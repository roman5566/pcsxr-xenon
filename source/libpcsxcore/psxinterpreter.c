...
int psxTestLoadDelay(int reg, u32 tmp) {
	...
	switch (tmp >> 26) {
		case 0x00: // SPECIAL
			switch (_tFunct_) {
				...
				case 0x02: case 0x03: // SRL/SRA
					if (_tRd_ == reg && _tRt_ == reg) return 1;
					else
				...
				case 0x08: // JR
					...
					break;
				case 0x09: // JALR
					if (_tRd_ == reg && _tRs_ == reg) return 1;
					else
						if (_tRs_ == reg) return 2;
					else
						if (_tRd_ == reg) return 3;
				...
				case 0x04: case 0x06: case 0x07: // SLLV...
					if (_tRd_ == reg && (_tRt_ == reg || _tRs_ == reg)) return 1;
					else
		...
		case 0x0c: case 0x0d: case 0x0e: // ADDI/ADDIU...
			if (_tRt_ == reg && _tRs_ == reg) return 1;
			else
				if (_tRs_ == reg) return 2;
			else
...
__inline void psxSLLV() {
	if (!_Rd_) return;
	_rRdU_ = _rRtU_ << Shamt();
} // Rd = Rt << rs

__inline void psxSRAV() {
	if (!_Rd_) return;
	_rRdS_ = _rRtS_ >> Shamt();
} // Rd = Rt >> rs (arithmetic)

__inline void psxSRLV() {
	if (!_Rd_) return;
	_rRdU_ = _rRtU_ >> Shamt();
} // Rd = Rt >> rs (logical)
...
void psxSB() {
	psxMemWrite8(_oB_, (psxRegs.GPR.p[_Rt_].b.l));
}

void psxSH() {
	psxMemWrite16(_oB_, (psxRegs.GPR.p[_Rt_].w.l));
}

void psxSW() {
	psxMemWrite32(_oB_, _rRtU_);
}
