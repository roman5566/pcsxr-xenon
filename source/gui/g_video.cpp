...
unsigned int * get_framebuffer_tiled() {
    struct ati_info *ai = (struct ati_info*) 0xec806100ULL;
    volatile unsigned int *screen = (unsigned int*) (long) (ai->base | 0x80000000);

    // Create surface
    unsigned int * surf = (unsigned int*) malloc(ai->height * ai->width * 4);

    // copy
    memcpy(surf, (void*)screen,ai->height * ai->width*4);

    return surf;
}

static void setTextureData(void * tex, void * buffer) {
