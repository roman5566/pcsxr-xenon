...
EXTERN long GPUopen(unsigned long * disp, char * CapText, char * CfgFile) {
	...
	peops_cfg.iResX = gpuRenderer.GetFramebufferWidth();
	peops_cfg.iResY = gpuRenderer.GetFramebufferHeight();

	InitializeTextureStore(); // init texture mem

	rRatioRect.left = rRatioRect.top = 0;
	rRatioRect.right = peops_cfg.iResX;
	rRatioRect.bottom = peops_cfg.iResY;
...
#define Vertex3f(i,_x,_y,_z)	\
	...
	gpuRenderer.primVertex(&vertex[i].x);

	if (PreviousPSXDisplay.Range.x0) {
		s = PreviousPSXDisplay.Range.x0 + 1;

		gpuRenderer.primBegin(PRIM_QUAD);
		Vertex3f(0, 0, 0, 0.99996f);
		Vertex3f(1, 0, PSXDisplay.DisplayMode.y, 0.99996f);
		Vertex3f(2, s, PSXDisplay.DisplayMode.y, 0.99996f);
		Vertex3f(3, s, 0, 0.99996f);
		gpuRenderer.primEnd();

		s += PreviousPSXDisplay.Range.x1 - 2;

		gpuRenderer.primBegin(PRIM_QUAD);
		Vertex3f(0, s, 0, 0.99996f);
		Vertex3f(1, s, PSXDisplay.DisplayMode.y, 0.99996f);
		Vertex3f(2, PSXDisplay.DisplayMode.x, PSXDisplay.DisplayMode.y, 0.99996f);
		Vertex3f(3, PSXDisplay.DisplayMode.x, 0, 0.99996f);
		gpuRenderer.primEnd();
	}
	...
	if (PreviousPSXDisplay.Range.y0) {
		...
		gpuRenderer.primBegin(PRIM_QUAD);
		Vertex3f(0, 0, 0, 0.99996f);
		Vertex3f(1, 0, s, 0.99996f);
		Vertex3f(2, PSXDisplay.DisplayMode.x, s, 0.99996f);
		Vertex3f(3, PSXDisplay.DisplayMode.x, 0, 0.99996f);
		gpuRenderer.primEnd();
	}
...
void updateDisplay(void) // UPDATE DISPLAY
{
	...
	if (iRenderFVR) // frame buffer read fix mode still active?
	{
		...
		if (!iRenderFVR) peops_cfg.bFullVRam = FALSE;
	}
	...
	if (peops_cfg.dwActFixes & 512) bCheckFF9G4(NULL); // special game fix for FF9
	...
	if (peops_cfg.dwActFixes & 128) // special FPS limitation mode?
	{
		if (peops_cfg.bUseFrameLimit) PCFrameCap(); // -> ok, do it
		if (peops_cfg.bUseFrameSkip)
			PCcalcfps();
	}

	if (bSnapShot) 
		DoSnapShot(); // snapshot key pressed? cheeeese :)


	//----------------------------------------------------//
	// main buffer swapping (well, or skip it)

	if (peops_cfg.bUseFrameSkip) // frame skipping active ?
	{
		if (!bSkipNextFrame) {
			if (iDrawnSomething)
				DoBufferSwap();
		}
		if (peops_cfg.dwActFixes & 0x180) // -> special old frame skipping: skip max one in a row
		{
...
void SetAspectRatio(void) {
	...
	xs = (float) peops_cfg.iResX / (float) PSXDisplay.DisplayModeNew.x;
	ys = (float) peops_cfg.iResY / (float) PSXDisplay.DisplayModeNew.y;
	...
	r.left = (peops_cfg.iResX - r.right) / 2;
	r.top = (peops_cfg.iResY - r.bottom) / 2;
	...
	if (r.bottom < rRatioRect.bottom ||
		...
		if (r.bottom < rRatioRect.bottom) {
			rC.left = 0;
			rC.top = 0;
			rC.right = peops_cfg.iResX;
			rC.bottom = r.top;
			//glScissor(rC.left, rC.top, rC.right, rC.bottom);
			gpuRenderer.SetScissor(rC.left, rC.top, rC.right, rC.bottom);
			gpuRenderer.Clear(uiBufferBits);
			rC.top = peops_cfg.iResY - rC.bottom;
			//glScissor(rC.left, rC.top, rC.right, rC.bottom);
			gpuRenderer.SetScissor(rC.left, rC.top, rC.right, rC.bottom);
			gpuRenderer.Clear(uiBufferBits);
		}
...
void updateDisplayIfChanged(void) {
	...
	} else // some res change?
	{
		...
		gpuRenderer.SetOrtho(0, PSXDisplay.DisplayModeNew.x, // -> new psx resolution
				PSXDisplay.DisplayModeNew.y, 0, -1, 1);
		if (peops_cfg.bKeepRatio) SetAspectRatio();
	}
	...
	if (peops_cfg.iFrameLimit == 2) SetAutoFrameCap(); // set new fps limit vals (depends on interlace)
...
EXTERN void CALLBACK GPUupdateLace(void) {
	...
	if (peops_cfg.iOffscreenDrawing == 4) // special check if high offscreen drawing is on
	{
...
EXTERN uint32_t CALLBACK GPUreadStatus(void) {
	if (peops_cfg.dwActFixes & 0x1000) // CC game fix
	{
...
EXTERN void CALLBACK GPUwriteStatus(uint32_t gdata) {
	...
	switch (lCommand) {
		...
		case 0x03:
			...
			if (peops_cfg.iOffscreenDrawing == 4 &&
...
void CheckVRamReadEx(int x, int y, int dx, int dy) {
	...
	if (dx > peops_cfg.iResX) dx = peops_cfg.iResX;
	if (dy > peops_cfg.iResY) dy = peops_cfg.iResY;

	if (dx <= 0) return;
	if (dy <= 0) return;

	// ogl y adjust
	y = peops_cfg.iResY - y - dy;

	x += rRatioRect.left;
	y -= rRatioRect.top;

	if (y < 0) y = 0;
	if ((y + dy) > peops_cfg.iResY) dy = peops_cfg.iResY - y;
...
void CheckVRamRead(int x, int y, int dx, int dy, BOOL bFront) {
	...
	if (!pGfxCardScreen) {
		//        glPixelStorei(GL_PACK_ALIGNMENT, 1);
		pGfxCardScreen = (unsigned char *) malloc(peops_cfg.iResX * peops_cfg.iResY * 4);
	}
...
EXTERN void CALLBACK GPUreadDataMem(uint32_t *pMem, int iSize) {
	...
	if ((peops_cfg.iFrameReadType & 1 && iSize > 1) &&
			!(iDrawnSomething == 2 &&
	...
	if (iDataWriteMode == DR_NORMAL) {
		for (; i < iSize;) {
			...
			if (gpuDataP == gpuDataC) {
				...
				if (dwEmuFixes & 0x0001 || peops_cfg.dwActFixes & 0x20000) // hack for emulating "gpu busy" in some games
					iFakePrimBusy = 4;
			}
		}
	}
...
void SetFixes(void) {
	...
	if (peops_cfg.dwActFixes & 0x2000)
		dispWidths[4] = 384;
...
EXTERN void CALLBACK GPUvisualVibration(uint32_t iSmall, uint32_t iBig) {
	...
	if (PSXDisplay.DisplayModeNew.x) // calc min "shake pixel" from screen width
		iVibVal = max(1, peops_cfg.iResX / PSXDisplay.DisplayModeNew.x);
	else iVibVal = 1;
