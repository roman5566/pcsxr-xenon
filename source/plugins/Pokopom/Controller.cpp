...
unsigned char Controller::command(const unsigned int counter, const unsigned char data) {
    ...
    switch (counter) {
        ...
        case 0x04: if (cmdBuffer[0] == 0x01) Cmd4(cmdBuffer[1]);
            break;
        case 0x08: if (cmdBuffer[0] == 0x01) Cmd8(cmdBuffer[1]);
            break;
    }
...
void Controller::Cmd0() {
    ...
    if (!bModeLock) {
        ...
        if (!bPressed & analogSwitch) {
            if (padID == ID_DIGITAL) {
                padID = ID_ANALOG;
                printf("Pokopom -> [0] Switched to analog mode.\n");
            } else if (padID == ID_ANALOG) {
                padID = ID_DIGITAL;
                printf("Pokopom -> [0] Switched to digital mode.\n");
            }
...
void Controller::Cmd4(const unsigned char data) {
    switch (data) {
        ...
        case 0x44: if (bConfig) {// Set mode and lock
                ...
                bModeLock = cmdBuffer[4] == 0x03;
            } // Disgaea sends 0x01 here
            break;
